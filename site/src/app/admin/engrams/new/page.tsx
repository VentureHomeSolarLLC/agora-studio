...
const STEPS = ['Basic Info', 'Skill Steps', 'Concepts', 'Lessons', 'Review'];
...
export default function NewEngramPage() {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (!data.success) {
        throw new Error(data.error || data.errors?.join(', ') || 'Failed to create engram');
      }
  ...
  if (submitResult) {
    return (
      <div className="max-w-2xl mx-auto py-12 text-center">
        ...
        <h1 className="text-2xl font-bold mb-4">Engram Created!</h1>
        ...
        <div className="flex gap-4 justify-center">
          <a href="/admin/engrams" className="bg-gray-900 text-white px-6 py-3 rounded-lg">Back to Engrams</a>
  ...
  return (
    <div className="max-w-4xl mx-auto py-8">
      <h1 className="text-2xl font-bold mb-8">Create New Engram</h1>
      <StepIndicator steps={STEPS} current={step} />
      {submitError && <div className="bg-red-50 text-red-700 px-4 py-3 rounded-lg mb-6">{submitError}</div>}
      
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        {step === 1 && <BasicInfoForm data={formData} onChange={updateFormData} />}
        {step === 2 && <SkillBuilder data={formData.skill} onChange={(skill) => updateFormData({ skill })} />}
        {step === 3 && <ConceptsBuilder data={formData.concepts || []} onChange={(concepts) => updateFormData({ concepts })} />}
        {step === 4 && <LessonsBuilder data={formData.lessons || []} onChange={(lessons) => updateFormData({ lessons })} />}
        {step === 5 && <ReviewAndPublish data={formData} onPublish={handlePublish} isSubmitting={isSubmitting} />}
      </div>

      <div className="flex justify-between mt-6">
        <button onClick={() => setStep(step - 1)} disabled={step === 1} className="px-6 py-3 text-gray-600 hover:bg-gray-100 rounded-lg disabled:opacity-30">Previous</button>
        {step < 5 && <button onClick={() => setStep(step + 1)} className="bg-gray-900 text-white px-6 py-3 rounded-lg hover:bg-gray-800">Next</button>}
      </div>
    </div>
  );
}
